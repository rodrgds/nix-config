...
Quickshell on NixOS and SketchyBar on macOS render the same compact Flexoki rail: workspaces and music on the left, system status and controls on the right. Shared dimensions, colors, fonts, and workspace metadata live in `modules/desktop-bar.nix`; each renderer keeps small native feature modules for platform behavior. SketchyBar mirrors the active macOS menu extras configured in `darwin.apps.sketchybar.trayAliases`; grant SketchyBar Screen & System Audio Recording access so aliases can render. Alias clicks open a stable app or Settings surface because macOS does not expose menu-extra popovers to SketchyBar. Vicinae shares extensions, behavior, favorites, font, and the Flexoki theme through `modules/apps/vicinae/default.nix`. `Super+D` / `Option+D` opens it and `Super+.` / `Option+.` opens emoji search; the laptop also replaces Spotlight's `Command+Space`. A rebuild applies bar or binding changes—no reboot is needed.
...
- `Win+Caps Lock` toggles the music scratchpad on the focused monitor. Spotify and the
