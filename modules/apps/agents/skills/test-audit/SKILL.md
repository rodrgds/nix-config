...
whole subsystem's test surface (every test file one plugin or core area owns);
before starting one, write down the subsystem scope and batch plan first.

When exploring the codebase, recall Hindsight for the project's verified
domain language and prior testing and interface decisions, then check them
against current code and deterministic repository contracts. When the shape of
a test boundary is itself in question, consult the `codebase-design` skill for
seam vocabulary and the `tdd` skill for loop rules. The owner boundary below
is that skill's seam: the public interface where behavior is observed without
reaching inside.
...
Run the closest existing checks first, using the repository's own runners and
policy. Do not start a file watcher in the checkout while editing source or
tests.

1. Run the smallest owner and sibling tests with the repo's test runner.
2. For removed source greps or plan assertions, run the executable script or
   dry-run that owns the real contract.
3. Run the repo's formatter on touched files, then `git diff --check`.
4. Run the repo's changed-file gate when one exists, then the actual CI checks
   required by repository policy for the changed paths.
5. Inspect `git diff --numstat`; report production and tooling separately from
   tests and test support.
6. After final audit edits, run the repo's standard review pass for the
   changed surfaces.

## Landing and continuation

Commit, push, open a PR, or land only when authorized. Follow the repository's
PR flow. Land one coherent PR at a time; after landing, refresh from current
